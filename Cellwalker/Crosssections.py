...
if os.name == 'posix':
    ...
    sys.path.append("C:\\Anaconda3\\envs\\blenderenv\\Lib\\site-packages")
else:
...
def create_planes_along_centerline(context, centerline, obj):
    ...
    out = open(bpy.context.scene.my_tool.path + "/" + object_name + ".cross-section_param.csv", 'w')
    out.write(
...
def make_plane(centerline, ind):
    ...
    norm_m1 = mathutils.Vector(norm_m1 / np.linalg.norm(norm_m1))
    norm_m2 = mathutils.Vector(norm_m2 / np.linalg.norm(norm_m2))
    norm_p1 = mathutils.Vector(norm_p1 / np.linalg.norm(norm_p1))
    norm_p2 = mathutils.Vector(norm_p2 / np.linalg.norm(norm_p2))
...
def calculate_cross_sectional(context):
    ...
    if centerline is None:
        ...
        if object_name in bpy.data.objects:
            ...
            create_planes_along_centerline(context, centerline, bpy.data.objects[object_name])
        else:
