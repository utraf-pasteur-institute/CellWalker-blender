...
class centerline(bpy.types.Operator):
    ...
    bl_description = "Create a centerline with distance of\n'step-size' units between consecutive points."
...
class export_operator(bpy.types.Operator):
    ...
    bl_description = "Calculate selected cross-section properties and save in a .CSV file"
...
class MySettings(bpy.types.PropertyGroup):
    path: bpy.props.StringProperty(
        name="",
        description="Path to a folder to save outout",
        default="",
        maxlen=1024,
        subtype='DIR_PATH'
    )
    ...
    slider: bpy.props.FloatProperty(
        name="Slider",
        description="Distance between consecutive points on the centerline.",
        default= 0.01,
        min =0.01,
        max = 5
    )
